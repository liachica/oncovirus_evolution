...
data_raw<-read.csv(file="output_lDNA_2_div1.csv")

data_raw$numGclasses<-2
...
ggplot(data, aes(log10(tfinal),colour=as.factor(numGclasses)))+
  scale_colour_viridis_d()+
  geom_density()


ggplot(data, aes(log10(trans+1),colour=as.factor(numGclasses)))+
  scale_colour_viridis_d()+
  geom_density()

ggplot(data, aes((mut>0),colour=as.factor(numGclasses)))+
  scale_colour_viridis_d()+
  geom_density()
...
for(WS in unique(data$numGclasses))
{
  
  {
    for(i1 in unique(data$epsilon1))
    {
      ...
      {
        temp<-subset(data,(numGclasses==WS)&(epsilon1==i1)&(epsilon2==i2))
...
write.csv(sumstats,file="sumstats_mean_lDNA2_1.csv",row.names = FALSE)
write.csv(sumstats_median,file="sumstats_median_lDNA2_1.csv",row.names = FALSE)
...
ggplot(subset(sumstats_median,(as.factor(numGclasses)==1)&(mut>0)), 
