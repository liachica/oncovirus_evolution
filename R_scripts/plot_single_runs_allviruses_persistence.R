...
gdata1<-read.csv(file = "single_run_lDNA_1_persist_(05-35).csv",header=T)
gdata2<-read.csv(file = "single_run_sDNA_persist_(05-35).csv",header=T)
gdata3<-read.csv(file = "single_run_retrovir_persist_(05-35).csv",header=T)
...
ggplot(subset(small_data3,(as.numeric(run)==c(1:3))),
  ...
  #  ylim(1,10)+
  #  guides(linetype=FALSE)+
  scale_y_log10(   breaks = c(1,10^2, 10^4, 10^6, 10^8, 10^10), labels = expression(1,10^2, 10^4, 10^6, 10^8, 10^10))+
...
ggsave(plot=last_plot(),filename="single_run_1_persist.pdf", width = 14, height = 5, units = "cm")
