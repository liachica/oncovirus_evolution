...
numGclasses=as.integer(8);     #number of classes for G to avoid rapid emergence of cancer "by chance"
...
plot(rep(-10,length(c(0:round(Tmax/365))))~c(0:round(Tmax/365)),type="l",xlab="time (years)",ylab="log[10](population size +1)",xlim=c(0,round(Tmax/365)),ylim=c(0,10))
for(j in c(1:10))
{
...
write.csv(gdata, file = "single_run_lDNA_8_cleared_(0-0).csv",row.names = F,quote=F)
